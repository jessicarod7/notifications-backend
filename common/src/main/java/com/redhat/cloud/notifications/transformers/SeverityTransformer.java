...
public class SeverityTransformer {
     ...
     * @return the determined severity level (which is also written to the top-level severity field of {@code data}
     */
    public Severity getSeverity(JsonObject data) {
        Severity severity;

        String severityField = data.getString(SEVERITY);
        if (!severityField.isEmpty()) {
            try {
                return Severity.valueOf(severityField.toUpperCase());
            } catch (IllegalArgumentException e) {
                severity = Severity.UNDEFINED;
                data.put(SEVERITY, Json.encode(severity));
            }
        } else {
            severity = extractLegacySeverity(data);
            data.put(SEVERITY, Json.encode(severity));
        }
