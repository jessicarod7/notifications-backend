...
class TestOcmTemplate {
    ...
    private static final String CLUSTER_MANAGER_DEFAULT_EVENT_URL = "https://cloud.redhat.com/openshift/details/s/" + OcmTestHelpers.SUBSCRIPTION_ID + "?from=notifications&integration=google_chat";
    ...
    private void checkResult(String eventType, String result, boolean multipleEvents) {
        if (eventType == null) {
            if (multipleEvents) {
                assertEquals("{\"text\":\"4 events have been triggered by cluster Atlantic.\\nThe first 3 events:" +
                ...
                assertEquals("{\"text\":\"1 event has been triggered by cluster Atlantic:\\n- *[" + OcmTestHelpers.DEFAULT_SEVERITY + "]* Subject line!" +
                        "\\n\\nView event details in <" + CLUSTER_MANAGER_DEFAULT_EVENT_URL + "|Cluster Manager - OpenShift>.\"}", result);
            }
        } else {
